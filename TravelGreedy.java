...
//this program solves travelling salesman problem with Greedy algorithm
public class TravelGreedy
{
  ...
  public TravelGreedy(int vertices, int[][] edges, PrintWriter PWOut)
  {
    ...
    while (!openList.isEmpty())						//while openList still has paths
    {
      ...
      else											//else if path is not complete
      {
		  ...
		  if (children != null)							//if there are children
		  {
			  ...
			  openList.addFirst(children.get(best));		//add child to list
		  }
      }
    }
  ...
  //method locates the path with the minimum cost in a list of paths
